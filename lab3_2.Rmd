...
```{r}
newObject2<- 
  fish %>% select(lakeid, radii_length_mm) %>% 
  filter((lakeid=="AL"|lakeid=="AR") &(radii_length_mm<= 4 & radii_length_mm>=2)) %>% 
...
```{r}
fish %>% 
  mutate(lengthHalf = length/2) %>% 
  select(fish_id, length, lengthHalf) %>% 
  arrange(desc(lengthHalf))
```
